...
const { PdpSimple} = require('../pageObjects/pdpSimple');
...
test.describe('Search Functionality', async function() {
    ...
    products.forEach(products => {
        test(`should return pdp as a search results for a product SKU: ${products.sku}`, async ({ page }) => {
            ...
            const pdpSimple = new PdpSimple(page);
            await page.goto('https://www.metaltis.be/');
            await header.searchField.isEnabled();
            await header.searchField.type(products.sku);
            await header.searchField.press('Enter');
            await expect(pdpSimple.sku).toContainText(products.sku);
        })
    })
